HELP = """
A script to introduce special Unicode characters,
primarily non-breaking spaces next to particles.

---

Examples:
  # Replace all Markdown files across all issues:
  pdm run particles **/*.md

  # Replace all Markdown files in the 0035-len/ directory
  pdm run particles 0035-len

  # Replace all Markdown files whose names start with ‘ni’ or ‘seme’:
  pdm run particles **/ni-*.md **/seme-*.md
"""
...
def improved_text_of(toki):
    ...
    # before vocative ‹o›
    toki, subs[1] = re.subn(
        r"(" + ilo_ala + r") (o\b" + kon_ken + ilo + r")", r"\1 \2", toki
    )
    ...
    toki, subs[3] = re.subn(
        r"(" + ilo + kon_ken + r"\ba) (" + ilo_ala + r")", r"\1 \2", toki
    )
    ...
    toki, subs[7] = re.subn(
        r"\bkijetesantakalu\b", "ki­je­te­san­ta­ka­lu", toki
    )
...
def several(counter, label):
    return f"{str(counter).rjust(3)} {label}{'' if counter == 1 else 's'}"
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        if not Path(MD_DIRECTORY + arg).is_dir():
            path = Path(MD_DIRECTORY + arg)
        else:
            path = Path(MD_DIRECTORY + arg) / "*.md"

        for file_name in glob.glob(str(path)):
            subs = improve_contents_of_file(file_name)
            total_subs += subs
            print(f"Applied {several(subs, "change")} in {file_name}.")
    print(f"(That’s {several(total_subs, "change")} altogether.)")
